...
func (b *Broker) Enqueue(j job.Job, ordered bool, maxFlight int) {
	b.mu.Lock()
	defer b.mu.Unlock()
	dq, ok := b.dests[j.StoreID]
	if !ok {
		dq = newDestQueue(j.StoreID, ordered, maxFlight)
		b.dests[j.StoreID] = dq
	}
...
func (b *Broker) Release(storeID string) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if dq, ok := b.dests[storeID]; ok && dq.inFlight > 0 {
		dq.inFlight--
	}
}
...
func (b *Broker) DepthByStore() map[string]int {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *Broker) Restore(jobs []job.Job, meta map[string]DestMeta) {
	...
	for _, j := range jobs {
		dq, ok := b.dests[j.StoreID]
		if !ok {
			dq = newDestQueue(j.StoreID, false, 2)
			b.dests[j.StoreID] = dq
		}
		heap.Push(&dq.ready, &item{j: j.Clone()})
	}
