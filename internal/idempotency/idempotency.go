...
// Remember returns the original frame id when the same key+hash is replayed.
// A live key with a different hash is a conflict.
func (s *Store) Remember(key, bodyHash, frameID string) (existingFrameID string, replay bool, err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.gcLocked()
	rec, ok := s.byKey[key]
