...
// Store is a downstream geology HTTP endpoint that receives WITS-like frames.
type Store struct {
	ID           string    `json:"id"`
...
func (in CreateInput) normalize() (CreateInput, error) {
	...
	for _, p := range in.TypePrefixes {
		...
		if err := frame.ValidateType(strings.TrimSuffix(p, ".")); err != nil && !strings.HasSuffix(p, ".") {
			stem := strings.TrimSuffix(p, ".")
			if err2 := frame.ValidateType(stem); err2 != nil {
...
func (d Store) Matches(frameType string) bool {
	if !d.Enabled {
		return false
	}
	for _, p := range d.TypePrefixes {
		if frame.MatchPrefix(frameType, p) {
			return true
		}
	}
	return false
...
func (d Store) OutboundSecrets(now time.Time) []string {
...
func NewRegistry(clk clock.Clock) *Registry {
	return &Registry{clk: clk, items: make(map[string]Store)}
}

func (r *Registry) Create(in CreateInput) (Store, error) {
	in, err := in.normalize()
	if err != nil {
		return Store{}, err
	}
...
func (r *Registry) Put(d Store) error {
	if d.ID == "" {
		return fmt.Errorf("store id required")
	}
...
func (r *Registry) SetEnabled(id string, enabled bool) (Store, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	d, ok := r.items[id]
	if !ok {
...
func (r *Registry) RotateSecret(id, next string, overlap time.Duration) (Store, error) {
	if len(next) < 8 {
		return Store{}, fmt.Errorf("secret must be at least 8 characters")
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	d, ok := r.items[id]
...
func (r *Registry) Matching(frameType string) []Store {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := make([]Store, 0)
	for _, d := range r.items {
...
func (r *Registry) Restore(items []Store) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.items = make(map[string]Store, len(items))
	for _, d := range items {
		r.items[d.ID] = d
	}
}

func (r *Registry) Public(d Store) map[string]any {
