...
func newTestEngine(t *testing.T, url string, clk clock.Clock) (*Engine, *geology.Store) {
	t.Helper()
	return newTestEngineOpts(t, url, clk, engineOpts{})
}

func newTestEngineOpts(t *testing.T, url string, clk clock.Clock, opts engineOpts) (*Engine, *geology.Store) {
	t.Helper()
	if opts.rate <= 0 {
		opts.rate = 100
	}
	...
	enabled := true
	d, err := stores.Create(geology.CreateInput{
		Name:         "t",
		URL:          url,
		Secret:       "abcdefgh",
		TypePrefixes: []string{""},
	...
	e := New(clk, broker, stores, gates, wits.New(opts.timeout), journal.New(50), dlq.New(50), backoff.Policy{
...
func sampleJob(storeID string, clk clock.Clock, relayID string) job.Job {
	return job.Job{
		FrameID:   "frm_test",
		RelayID:   relayID,
		StoreID:   storeID,
		Type:      "lithology.cuttings",
		Body:      []byte(`{"type":"lithology.cuttings","payload":{"rop_m_hr":12}}`),
		Attempt:   0,
		NotBefore: clk.Now(),
		CreatedAt: clk.Now(),
	}
...
func TestHandleRetriesStatus429(t *testing.T) {
	...
	j := sampleJob(dest.ID, clk, "rly_test429")
...
func TestHandleTreats202AsSuccess(t *testing.T) {
	...
	clk := clock.NewFrozen(time.Unix(1_700_000_000, 0))
	e, dest := newTestEngine(t, srv.URL, clk)
	e.handle(context.Background(), sampleJob(dest.ID, clk, "rly_test202"))
	if e.dead.Len() != 0 {
...
func TestHandleHonorsCancel(t *testing.T) {
	...
	go func() {
		defer close(done)
		e.handle(ctx, sampleJob(dest.ID, clk, "rly_cancel"))
	}()
	select {
	case <-accepted:
...
func TestHandleTimeoutIsRetryable(t *testing.T) {
	...
	e.handle(context.Background(), sampleJob(dest.ID, clk, "rly_timeout"))
	if e.dead.Len() != 0 {
...
func TestHandleSuccessClearsBreaker(t *testing.T) {
	...
	e.handle(context.Background(), sampleJob(dest.ID, clk, "rly_brk_1"))
	e.handle(context.Background(), sampleJob(dest.ID, clk, "rly_brk_2"))
	e.handle(context.Background(), sampleJob(dest.ID, clk, "rly_brk_3"))
...
func TestHandleRespectsRateLimit(t *testing.T) {
	...
	e.handle(context.Background(), sampleJob(dest.ID, clk, "rly_rl_1"))
	e.handle(context.Background(), sampleJob(dest.ID, clk, "rly_rl_2"))
	if hits.Load() != 1 {
